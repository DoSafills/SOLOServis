...
export function Rating({ value, count }: { value: number; count?: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex">
        {[1, 2, 3, 4, 5].map((i) => (
          <svg key={i} width="12" height="12" viewBox="0 0 24 24" fill={i <= Math.round(safeValue) ? '#FBBF24' : '#2A2A2A'}>
            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
          </svg>
        ))}
      </div>
      <span className="text-xs font-semibold text-warn">{safeValue.toFixed(1)}</span>
      {safeCount !== undefined && <span className="text-xs text-muted">({safeCount.toLocaleString('es-CL')})</span>}
    </div>
  )
}
