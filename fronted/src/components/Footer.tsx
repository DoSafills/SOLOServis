        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-xs text-muted leading-relaxed">
              Compara precios entre cientos de tiendas y proveedores. Encuentra siempre la mejor
              oferta.
            </p>
            <div className="flex gap-3 mt-4">
              {["Twitter", "Instagram", "Facebook"].map((s) => (
                <button
                  key={s}
                  className="w-8 h-8 rounded-lg bg-surface-2 text-muted hover:text-prime hover:bg-prime-muted transition-all flex items-center justify-center text-xs font-bold"
                >
                  {s[0]}
                </button>
              ))}
          ...
          {/* Links */}
          <div>
            <h4 className="text-xs font-semibold text-muted-2 uppercase tracking-widest mb-4">
              Plataforma
            </h4>
            <ul className="space-y-2.5">
              {["Sobre nosotros", "Cómo funciona", "Para tiendas", "Blog"].map((l) => (
                <li key={l}>
                  <button className="text-sm text-muted hover:text-prime transition-colors">
                    {l}
                  </button>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {["Centro de ayuda", "Preguntas frecuentes", "Contacto", "Reportar error"].map(
                (l) => (
                  <li key={l}>
                    <button className="text-sm text-muted hover:text-prime transition-colors">
                      {l}
