    ...
    <div style={{ background: '#111111', border: '1px solid #2A2A2A' }} className="rounded-2xl p-6">
            ...
            <button
              ...
              style={
                mode === 'offer'
                  ...
                  : hasOfferHistory
                    ? { background: '#1A1A1A', border: '1px solid #2A2A2A', color: '#64748B' }
                    : { background: '#111111', border: '1px solid #1A1A1A', color: '#2A2A2A', cursor: 'not-allowed' }
              }
              className="px-3 py-1 rounded-lg text-xs font-semibold transition-all"
            >
              Precio oferta
      ...
      {mode === 'offer' && !hasOfferHistory && (
        ...
        >
          ...
          <p className="text-xs text-muted max-w-xs mx-auto">
            Este producto no ha registrado precios de oferta. Puedes agregarlo a favoritos para recibir una alerta cuando haya una promoción.
          </p>
        </div>
      )}
      ...
      {mode === 'offer' && hasOfferHistory && currentOfferPrice && (
        ...
        >
          ...
          <span className="price text-sm font-bold text-prime">{formatPrice(currentOfferPrice)}</span>
      ...
      {activeData.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            { label: mode === 'offer' ? 'Última oferta' : 'Precio actual', value: formatPrice(currentPrice), color: 'text-prime' },
