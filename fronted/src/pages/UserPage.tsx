...
import { getProducts, formatPrice, getMinPrice } from '../Services/api/frontend-src/api'
  ...
  useEffect(() => {
    ...
    async function loadWatched() {
      ...
      try {
        ...
        if (cancelled) return
        setWatched(result.slice(0, 3))
      } catch (err) {
        if (cancelled) return
        setWatchedError(err instanceof Error ? err.message : 'Error al cargar los productos observados')
      } finally {
          ...
          {activeTab === 'watched' && (
            <div style={{ background: '#111111', border: '1px solid #2A2A2A' }} className="rounded-2xl p-6">
              ...
              {watchedLoading ? (
                ...
                <p className="text-sm text-warn">{watchedError}</p>
              ) : (
