...
export default function HomePage({ navigate, favorites, compareList, onToggleFavorite, onToggleCompare }: Props) {
  ...
  return (
    <div>
      ...
      <section
        style={{
          background: 'radial-gradient(ellipse 80% 60% at 50% -10%, rgba(232,0,27,0.15) 0%, transparent 70%), #0A0A0A',
          borderBottom: '1px solid #2A2A2A',
        }}
        className="py-20 px-4"
      >
        <div className="max-w-3xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-6" style={{ background: 'rgba(232,0,27,0.12)', border: '1px solid rgba(232,0,27,0.3)' }}>
            <span className="w-1.5 h-1.5 rounded-full bg-prime animate-pulse" />
            <span className="text-xs font-semibold text-prime">+2.400 tiendas comparadas en tiempo real</span>
          </div>
          ...
          <p className="text-base text-muted max-w-lg mx-auto mb-10">
            Busca productos y servicios entre cientos de tiendas y proveedores. Siempre el precio más bajo.
          </p>
      ...
      {/* Stats */}
      <section style={{ background: '#0A0A0A', borderBottom: '1px solid #2A2A2A' }} className="py-8 px-4">
      ...
      <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {productCategories.map((cat) => (
              ...
              >
                ...
                <span className="text-xs font-medium text-muted-2 group-hover:text-prime transition-colors">{cat.name}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="pb-12">
          ...
          {loading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (
                <div key={i} style={{ background: '#111111', border: '1px solid #2A2A2A' }} className="rounded-2xl h-64 animate-pulse" />
              ))}
            </div>
          )}

          {!loading && error && (
            <p className="text-sm text-red-400">No pudimos cargar los productos destacados: {error}</p>
          )}
        ...
        <section className="pb-16">
          ...
          {!loading && error && (
            <p className="text-sm text-red-400">No pudimos cargar los servicios destacados: {error}</p>
          )}
