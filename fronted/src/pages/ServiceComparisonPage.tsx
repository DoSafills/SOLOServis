...
export default function ServiceComparisonPage({ serviceIds, navigate }: Props) {
  const selected = serviceIds.map((id) => services.find((s) => s.id === id)).filter(Boolean) as typeof services
  ...
  const compareRows: { key: string; label: string; getValue: (s: typeof services[0]) => string; highlight?: (values: string[]) => string | null }[] = [
    {
      key: 'monthlyPrice',
