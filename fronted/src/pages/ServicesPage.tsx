...
export default function ServicesPage({ query, navigate, favorites, compareList, onToggleFavorite, onToggleCompare }: Props) {
  ...
  let filtered = services.filter((s) => {
    if (query && !s.name.toLowerCase().includes(query.toLowerCase()) && !s.provider.toLowerCase().includes(query.toLowerCase()) && !s.category.toLowerCase().includes(query.toLowerCase())) return false
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-6">
        {/* Sidebar */}
        <aside style={{ background: '#111111', border: '1px solid #2A2A2A' }} className="hidden lg:block w-52 shrink-0 rounded-2xl p-5 self-start sticky top-24">
        ...
        <div className="flex-1 min-w-0">
          {filtered.length === 0 ? (
            <EmptyState
              ...
              action={{ label: 'Ver todos los servicios', onClick: () => navigate({ id: 'search-services', query: '' }) }}
            />
          ) : (
