...
const specRows = ['VRAM', 'Arquitectura', 'Núcleos CUDA', 'Stream Processors', 'Bus de memoria', 'TDP', 'Garantía', 'Conectores', 'Procesador', 'RAM', 'Almacenamiento', 'Pantalla', 'Sistema operativo']

export default function ProductComparisonPage({ productIds, navigate }: Props) {
  const selected = productIds.map((id) => products.find((p) => p.id === id)).filter(Boolean) as typeof products

  if (selected.length < 2) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <p className="text-muted mb-4">Selecciona al menos 2 productos para comparar.</p>
        <button onClick={() => navigate({ id: 'search-products', query: '' })} style={{ background: '#E8001B', color: '#0A0A0A' }} className="px-5 py-2 rounded-xl text-sm font-semibold">
          Buscar productos
        </button>
      </div>
    )
  ...
  const allSpecKeys = specRows.filter((key) =>
    selected.some((p) => p.specs[key] !== undefined)
  )
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[560px]">
          ...
          <thead>
            <tr>
              <th
                style={{ background: '#111111', borderBottom: '1px solid #2A2A2A', borderRight: '1px solid #2A2A2A' }}
              ...
              {selected.map((p) => (
                <th
                  key={p.id}
                  style={{ background: '#111111', borderBottom: '1px solid #2A2A2A', borderRight: '1px solid #1A1A1A' }}
          ...
          <tbody>
            ...
            <tr style={{ background: 'rgba(34,211,160,0.04)' }}>
              ...
              {selected.map((p, i) => (
                <td key={p.id} style={{ borderBottom: '1px solid #1A1A1A', borderRight: '1px solid #1A1A1A' }} className="p-4 text-center">
                  <div className={`price text-lg font-bold ${minPrices[i] === lowestPrice ? 'text-prime' : 'text-text'}`}>
            ...
            <tr>
              ...
              {selected.map((p) => {
                ...
                return (
                  <td key={p.id} style={{ borderBottom: '1px solid #1A1A1A', borderRight: '1px solid #1A1A1A' }} className="p-4 text-center">
                    <span className={`text-sm font-bold ${p.rating === best ? 'text-warn' : 'text-text'}`}>
                      ★ {p.rating.toFixed(1)}
                    </span>
                    <div className="text-xs text-muted">({p.reviewCount.toLocaleString('es-CL')})</div>
                  </td>
                )
              })}
            </tr>
            ...
            {allSpecKeys.map((key, ri) => (
              <tr key={key} style={{ background: ri % 2 === 0 ? '#0A0A0A' : 'transparent' }}>
                ...
                {selected.map((p) => (
                  <td key={p.id} style={{ borderBottom: '1px solid #1A1A1A', borderRight: '1px solid #1A1A1A' }} className="p-4 text-center">
                    <span className="text-sm text-text">{p.specs[key] ?? <span className="text-muted">—</span>}</span>
                  </td>
                ))}
              </tr>
            ))}
            ...
            {/* Availability */}
            <tr>
              <td style={{ borderRight: '1px solid #2A2A2A' }} className="p-4 text-xs font-semibold text-muted-2 uppercase tracking-wide">
                Tiendas
              </td>
              {selected.map((p) => (
                <td key={p.id} style={{ borderRight: '1px solid #1A1A1A' }} className="p-4 text-center">
