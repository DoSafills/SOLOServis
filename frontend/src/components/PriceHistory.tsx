  ...
  if (active && payload && payload.length) {
    return (
      ...
      >
        ...
        <p className="price text-sm font-semibold text-prime">
...
export default function PriceHistory({
  ...
  const currentPrice = activeData.length
    ? activeData[activeData.length - 1].price
    : 0
      ...
      {mode === "offer" && !hasOfferHistory && (
        ...
        >
          ...
          <p className="text-sm font-semibold text-text mb-1">
            No ha habido una oferta
          </p>
          <p className="text-xs text-muted max-w-xs mx-auto">
            Este producto no ha registrado precios de oferta. Puedes agregarlo a
            favoritos para recibir una alerta cuando haya una promoción.
          </p>
        </div>
      )}
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            >
              ...
              <div className={`price text-sm font-semibold ${stat.color}`}>
                {stat.value}
              </div>
            </div>
          ))}
